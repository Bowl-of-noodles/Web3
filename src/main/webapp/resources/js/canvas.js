...
function drawCanvas() {
    ...
    ctx.globalAlpha = 0.4;
    ctx.fillRect(xAxis - radius * xNameAxis, yAxis, radius * xNameAxis, radius/2 * yNameAxis);
    ctx.fillStyle = "#9933ff";
    ctx.beginPath();
    ctx.moveTo(xAxis, yAxis);
    ctx.lineTo(xAxis - radius/2 * xNameAxis, yAxis);
    ctx.lineTo(xAxis, yAxis - radius/2 * yNameAxis);
    ctx.fill();
    ctx.closePath();
    ctx.beginPath();
    ctx.moveTo(xAxis, yAxis);
    ctx.fillStyle = "#9933ff";
    ctx.arc(xAxis, yAxis, xAxis - (6 - radius/2) * xNameAxis, Math.PI * 1.5, Math.PI * 2);
    ctx.fill();
    ctx.closePath();
}
